 ...
 *   Only a session whose agent is actively RUNNING is refused; an idle session
 *   that merely remains loaded (live) in the host is deletable.
 ...
 *   tool results. Message deletion still refuses ANY live session because the
 *   append bypasses the live session's in-memory surface.
 */

import { rm } from 'node:fs/promises'
import { dirname } from 'node:path'
import path from 'node:path' // for normalize in path-fence
...
async function handleDeleteSession(ctx, req, res) {
  ...
  // "Running" here means the session's agent is actively processing a turn.
  // A session can be a live (loaded) session yet idle and non-current — DSH keeps
  // such sessions open until the process exits, so blocking on "live" would make
  // every used session permanently undeletable. Only reject sessions whose agent
  // is actually running (mid-turn), matching the browser's delete-button rule.
  // The agent registry keys agents by session id (agent.id === session.id), so
  // `agents.get(sessionId)` resolves the exact agent for this session.
  ...
  if (agents !== undefined) {
    ...
    // Without the agent registry we cannot tell idle from running; fall back to
    // the live-session store so we never delete a session that may be in use.
    // (In practice the agent loop is always present in a web profile.)
    const sessions = ctx.get('sessions')
    running = sessions !== undefined && sessions.get(sessionId) !== undefined
  }
  if (running) {
  ...
  // We abort on failure (B3): archiving is the observable "this session is
  // gone" signal. If the archive throws, returning here leaves the log on disk
  // and the session live in the sidebar — consistent. Continuing past a
  // failed archive was the old behaviour, and it produced the inconsistency
  // where the log was gone but the sidebar entry remained.
  ...
  if (registry !== undefined) {
    try {
      ...
      return writeJson(res, 500, {
        ...
        message: '归档会话失败：' + String(error) + '；未执行物理删除，会话状态保持一致',
      })
    }
  }
  ...
  try { location = persistence.locate(meta) } catch (error) {
    // locate() failure used to be silently swallowed (B2). The session was
    // archived but the log could not be touched; the user got a misleading
    // 200. Surface it as a warning so the client can tell the user.
    return writeJson(res, 200, {
      ok: true,
      sessionId,
      warnings: ['locate-failed: ' + String(error)],
    })
  }
  if (location !== undefined && location.path !== undefined) {
    const dir = dirname(String(location.path))
    if (dir !== '' && dir !== '/' && dir !== '.') {
      // B2: only delete when the directory is a strict child of the session
      // root. A misconfigured persistence backend that returned an out-of-tree
      // path (e.g. "/etc") would otherwise be rm-ed wholesale. `locateRoot`
      // may be undefined for backends that don't expose it; in that case we
      // skip the fence and trust the backend (the prior behaviour), but log
      // a warning so an operator notices the missing root.
      ...
      const inTree = locateRoot === undefined
        || (() => {
          // Normalize separators on both sides: locate() and locateRoot() may
          // each return forward or back slashes (the underlying store decides),
          // and a naive startsWith would refuse a legitimate path that just
          // happens to mix them. Use posix.normalize so the comparison is
          // platform-independent — what matters here is the textual tree,
          // not the OS.
          const normalize = (s) => path.posix.normalize(String(s).replace(/[\\/]+/g, '/')).replace(/[\\/]+$/, '')
      ...
      if (!inTree) {
        return writeJson(res, 500, {
          ok: false,
          error: 'unsafe-location',
          message: '持久化返回的目录不在会话数据根目录之内，已中止物理删除',
        })
      }
      try {
        // Async so a large directory removal does not block the host event
        // loop (the host serves other API routes concurrently). The rm impl
        // is read from ctx.get('fsRm') when one is injected (used in tests
        // to assert the deletion is or isn't attempted); otherwise the
        // real fs.promises.rm is used.
        const rmImpl = (ctx.get && typeof ctx.get('fsRm') === 'function') ? ctx.get('fsRm') : rm
      ...
      if (rootWarn !== null) {
        return writeJson(res, 200, { ok: true, sessionId, warnings: [rootWarn] })
      }
    }
  }
...
async function handleDeleteMessage(ctx, req, res) {
  ...
  const sessions = ctx.get('sessions')
  const persistence = ctx.get('sessionPersistence')
  // Message deletion stays stricter than whole-session deletion: `persistence.append`
  // writes straight to the log, bypassing the live session's in-memory surface, so
  // appending to any live session would diverge its in-memory state. Refuse live
  // sessions here (idle or running); whole-session delete only refuses running ones.
