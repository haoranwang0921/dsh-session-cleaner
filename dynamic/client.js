...
// DeepSeek Harness dynamic Cordis plugin. This file is the `code.client` body:
...
return {
  ...
  apply(ctx) {
    ...
    const stop = ctx.slots.inject('settings.section', () => ctx.slots.register({
      name: 'settings.section',
      id: 'conversation-cleaner',
      order: 30,
      label: () => '会话管理',
      inject: () => ({
        remove: async (sessionId) => host.call('delete-session', { sessionId }),
        listMessages: async (sessionId) => host.call('list-messages', { sessionId }),
        removeMessage: async (sessionId, seq) => host.call('delete-message', { sessionId, seq }),
      }),
    }, SessionCleaner))

    return () => { stop(); stylesDispose() }
  },
}
...
function SessionCleaner(props) {
  ...
  const doDelete = async (id) => {
    ...
    setMessage(null)
    try {
      const result = await props.remove(id)
  ...
  const removeMessage = async (id, seq, role) => {
    ...
    setMsgError(null)
    try {
      const result = await props.removeMessage(id, seq)
      if (result && result.ok === true) {
